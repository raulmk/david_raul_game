...
def cinematica():
    ...
    sprite_fondo.set_position(81, 20)
    ...
    sprite_fondo.set_position(81, 60)
...
def help2():
    ...
    sprite_fondo = sprites.create(assets.image("""
        logo
    """), SpriteKind.Fondo)
    sprite_fondo.change_scale(3, ScaleAnchor.MIDDLE)
    sprite_fondo.set_position(80, 45)
    ...
    sprite_fondo = sprites.create(assets.image("""
        boton_a
    """), SpriteKind.Fondo)
    sprite_fondo.change_scale(1, ScaleAnchor.MIDDLE)
    sprite_fondo.set_position(80, 45)
    game.show_long_text("", DialogLayout.BOTTOM)
    ...
    sprite_fondo = sprites.create(assets.image("""
        logo
    """), SpriteKind.Fondo)
    sprite_fondo.change_scale(2, ScaleAnchor.MIDDLE)
    sprite_fondo.set_position(80, 45)
...
def Level_Control():
    global sprite_fondo, Play, Help, lore, Selector, level_game
    if level == 0:
        ...
        controller.move_sprite(Selector)
    elif level == 1:
        music.stop_all_sounds()
        sprites.destroy_all_sprites_of_kind(SpriteKind.Fondo)
...
def level_game_cntrl():
    global Raimon, lava_level, sprite_fondo
    if level_game == 1:
        ...
        Raimon = sprites.create(assets.image("""
            user
        """), SpriteKind.player)
        tiles.place_on_random_tile(Raimon, assets.tile("""
            trueinicio2
        """))
        controller.move_sprite(Raimon, 150, 0)
        Raimon.ay = 800
        # tiles.place_on_random_tile(mySprite, img("""
        # trueinicio1
        # """))
        lava_level = 0
        scene.camera_follow_sprite(Raimon)
    elif level_game == 2:
        music.stop_all_sounds()
        music.play(music.create_song(assets.song("""
                musica_tierra
        ...
        sprite_fondo.set_scale(1, ScaleAnchor.MIDDLE)
        game.show_long_text("DIOS: HOLA HIJO MÍO...", DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_raimon()
        sprite_fondo.set_position(81, 20)
        game.show_long_text("RAIMON: ¿Messi eres tú?", DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_dios()
        sprite_fondo.set_scale(1, ScaleAnchor.MIDDLE)
        game.show_long_text("DIOS: ¿QUIÉN SINÓ?", DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_raimon()
        sprite_fondo.set_position(81, 20)
        game.show_long_text("RAIMON: oh dios mío... mmm esto si que no me lo esperaba...",
            DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_dios()
        sprite_fondo.set_scale(1, ScaleAnchor.MIDDLE)
        game.show_long_text("DIOS: LO SÉ... RECUERDO TUS LLANTOS DE FELICIDAD CUANDO EL BARÇA GANÓ EL SEXTETE. SOS UN FIEL CREYENTE POR ESO TE ABRÍ LAS PUERTAS DEL CIELO.",
            DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_raimon()
        sprite_fondo.set_position(81, 20)
        game.show_long_text("RAIMON: SIEMPRE HE SIDO FAN DE TI! Por favor, necesito tu ayuda. Verás...",
            DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_dios()
        sprite_fondo.set_scale(1, ScaleAnchor.MIDDLE)
        game.show_long_text("DIOS: LO SÉ TODO. NO TE PREOCUPÉS YO ME ENCARGO HACETE A UN LADO.",
            DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_bobo()
        sprite_fondo.set_scale(1, ScaleAnchor.MIDDLE)
        game.show_long_text("DIOS: ANDÁ PA ALLÁ BOOOBO !", DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_infierno()
        sprite_fondo.set_position(81, 60)
        game.show_long_text("El Diablo desciende asustado hacia el infierno al escuchar a DIOS.",
            DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_dios()
        sprite_fondo.set_scale(1, ScaleAnchor.MIDDLE)
        game.show_long_text("DIOS: YA SE FUE EL PELOTUDO, AHORA ESCUCHÁME. RAIMON ERES CULÉ, NO HAY MEJOR PERSONA QUE VOS. CONFIÁ EN TI, ESCUCHÁ A TUS ALUMNOS Y SIGUE SIENDO COMO SOS. VALÉS MUCHO.",
            DialogLayout.BOTTOM)
        sprite_fondo = create_sprite_raimon()
        sprite_fondo.set_position(81, 20)
        game.show_long_text("RAIMON: Tienes razón Leo...", DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_dios()
        sprite_fondo.set_scale(1, ScaleAnchor.MIDDLE)
        game.show_long_text("DIOS: DIOS. LEO ES COMO ME LLAMAN EN LA TIERRA.",
            DialogLayout.BOTTOM)
        sprites.destroy(sprite_fondo)
        sprite_fondo = create_sprite_raimon()
        sprite_fondo.set_position(81, 20)
        game.show_long_text("RAMON: Perdón. Te prometo que a partir de ahora escucharé más a mis alumnos. Seré...",
            DialogLayout.BOTTOM)
        game.set_game_over_message(True, "¡EL MEJOR PROFE!")
        game.game_over(True)
rayo: Sprite = None
paloma: Sprite = None
...
def on_forever():
    ...
    if level == 1:
        Raimon.ay = 1000
        if Raimon.is_hitting_tile(CollisionDirection.BOTTOM):
            saltos = 2
        if info.life() == 0:
            if level_game == 1:
